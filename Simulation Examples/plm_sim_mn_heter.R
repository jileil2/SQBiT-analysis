...
# heteroskeedastic case
# MN Nois
mus <- c(-5, 0, 5)
sigmas <- c(2, sqrt(1/2), 2)
W <- c(.1, .8, .1)
require('nleqslv')
target <- function(x) {
  pnormm(q = x, p = W, mu = mus, sigma = sigmas) - tau
}
Finv <- nleqslv(x = 1, target)$x 
...
for (c in cs) {
  ...
  for (i in 1:nsim) {
    ...
    C <- C - 0.5 * sqrt(3)
    X <- runif(n, -1, 1)
    Y <- beta0[, 1] + X * beta0[, 2] + C %*% eta + (1 + .2 * C[, 3] + X * x1(S[, 'u'], S[, 'v'])) * rnormm(n = n, p = W, mu = mus, sigma = sigmas)
    X <- cbind(1, X)  
...
tikz(paste0('plm_mn_heter_n', n, '_truth_beta0_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
...
tikz(paste0('plm_mn_heter_n', n, '_truth_beta1_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
int <- interp(x = pop.r[, 1], y = pop.r[, 2], duplicate = "mean",
...
# qplvcm beta0
tikz(paste0('plm_mn_heter_n', n, '_QBiT_beta0_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
...
# qplvcm beta1
tikz(paste0('plm_mn_heter_n', n, '_QBiT_beta1_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
int <- interp(x = S.pop[, 1], y = S.pop[, 2], duplicate = "mean",
...
# sqplvcm beta0
tikz(paste0('plm_mn_heter_n', n, '_SQBiT_beta0_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
int <- interp(x = S.pop[, 1], y = S.pop[, 2], duplicate = "mean",
...
# sqplvcm beta1
tikz(paste0('plm_mn_heter_n', n, '_SQBiT_beta1_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
int <- interp(x = S.pop[, 1], y = S.pop[, 2], duplicate = "mean",
...
# tps beta0
tikz(paste0('plm_mn_heter_n', n, '_tps_beta0_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
...
# tps beta1
tikz(paste0('plm_mn_heter_n', n, '_tps_beta1_tau', tau * 100, '.tex'), width = 4, height = 3, standAlone = TRUE)
int <- interp(x = S.pop[, 1], y = S.pop[, 2], duplicate = "mean",
